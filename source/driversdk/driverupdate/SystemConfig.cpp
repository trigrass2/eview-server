...
{		
	...
	if(szCVProjCfgPath)
		sprintf(szConfigFile, "%s%c%s",szCVProjCfgPath, ACE_DIRECTORY_SEPARATOR_CHAR, "driverupdate.xml");
	else
		sprintf(szConfigFile, "%s", "driverupdate.xml");
	...
	// 2.Get Root Element.
	TiXmlElement* pElmRoot = doc.RootElement(); // "driverupdate" node
	if(!pElmRoot)
	{
	...
	if(!pNodeServer)
	{
		PKLog.LogMessage(PK_LOGLEVEL_ERROR, "Inner forward Driver Config File %s has no driverupdate/server node, errCode=%d!", szConfigFile, EC_ICV_DA_CONFIG_FILE_INVALID);
		return false;
	}
	...
	{
		PKLog.LogMessage(PK_LOGLEVEL_ERROR, "DriverUpdater Config File %s has no driverupdate/server::ip is null, errCode=%d!", 
			szConfigFile, EC_ICV_DA_CONFIG_FILE_INVALID);
		return false;
	}
	if(m_nServerPort <= 0)
	{
		PKLog.LogMessage(PK_LOGLEVEL_NOTICE, "DriverUpdater Config File %s driverupdate/server::port(%s) is null or negative , errCode=%d! default to 8890", 
	...
	{
		PKLog.LogMessage(PK_LOGLEVEL_NOTICE, "DriverUpdater Config File %s driverupdate/server::cycle_ms(%s) is null or negative , errCode=%d! default to 1 minutes(1000*60ms)", 
