...
{	
	...
	while(!m_bStop)
	{
		...
		while(!m_bStop)
		{
			...
			PKLog.LogMessage(PK_LOGLEVEL_INFO, "CtrlRouteTask recv an controls msg!");
			Json::Reader reader;
			Json::Value root;
			if (!reader.parse(strMessage, root, false))
			{
				PKLog.LogMessage(PK_LOGLEVEL_ERROR, "CtrlRouteTask recv an msg(%s), invalid!",strMessage.c_str());
			...
			{
				PKLog.LogMessage(PK_LOGLEVEL_ERROR, "CtrlRouteTask recv an msg(%s), not an object or array!",strMessage.c_str());
				g_pServer->UpdateServerStatus(-2,"control msg format invalid");
				continue;
			}
	...
	PKLog.LogMessage(PK_LOGLEVEL_NOTICE,"CtrlRouteTask 正常退出！");
	OnStop();
...
int CCtrlRouteTask::RouteOrSetAlarm(string &strCtrlCmd, Json::Value &root)
{
	int nRet = PK_SUCCESS;
