...
public:
	BOOL ShowProcessWindow(DWORD pid, int nShowFlag = SW_SHOW);
	CFindKillProcess();
	~CFindKillProcess();
	DWORD GetProcessIDOnlyWin32(LPCTSTR lpModname, BOOL bAddExe = TRUE);
	DWORD GetProcessID(LPCTSTR szProcessName, BOOL bAddExeExt = TRUE);
	void GetProcessInfo(map<string, DWORD> &mapProcessName2Id);
	BOOL  KillProcess(DWORD pid, BOOL bZap,DWORD dwWaitTime = 3000);//dwWaitTime强制关闭时的等待时间
	CString GetFileName(CString strPath);
	BOOL m_bOnlyWaitOnceOnKillProcess;//关闭多个进程时，在关闭进程之前仅仅等待一次
	BOOL m_bFirstWaitKillProcess;//是否第一次关闭进程
};
