...
void CFindKillProcess::GetProcessInfo(map<string, DWORD> &mapProcessName2Id)
{
	...
	if (((int)toolHelp32Snapshot) != -1)
	{
		...
		if (Process32First(toolHelp32Snapshot, &processEntry32))
		{
			do
			{
				string strExeFileName = processEntry32.szExeFile;
				transform(strExeFileName.begin(), strExeFileName.end(), strExeFileName.begin(), ::tolower);// 转为小写
				if (mapProcessName2Id.find(strExeFileName) == mapProcessName2Id.end())
					mapProcessName2Id[strExeFileName] = processEntry32.th32ProcessID;
			} while (Process32Next(toolHelp32Snapshot, &processEntry32));
		}
		CloseHandle(toolHelp32Snapshot);
	}
