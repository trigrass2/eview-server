...
#define TOPIC_CONFIG_PREFIX		"config"				// 配置通道;
...
int CMainTask::svc()
{
	...
	while (!m_bStop)
	{
		...
		//first time
		if (nTimeSpanSec >= 5) // 每60秒发送1次全量数据;
		{
		...
		{
			vector<TAGINFO> vecEventTag;		//事件相关的点位信息;
			vector<string> vecEventTagValues;	//事件相关点位的值;
			vector<TAGINFO> vUpdateTag;			//数据发生变化的点位信息;
			vector<string> vUpdateValue;		//发生变化点位的值;
			map<string, int> mState;			//各事件State改变情况
			map<string, int> mCode;				//各事件Code改变情况

			GetEventDetails(vecEventTag, vecEventTagValues, mState, mCode, vecTagsValue); //获取事件相关的详细信息;
			//将数据排序,与map中的状态点对应;
...
int CMainTask::ReadAllTags()
{
	...
	for (int i = 0; i < m_deviceEvent.size(); i++)	//根据事件设备数量定义事件code数据集;
	{
...
int CMainTask::LoadConfig()
{
	...
	if (nRet != 0)
	{
		g_logger.LogMessage(PK_LOGLEVEL_CRITICAL, "网关配置表 t_forward_list表 中查询异常：sql：%s, error:%s", szSql, strError.c_str());
		return -1;
	}
	...
	if (vecRows.size() <= 0)
	{
		g_logger.LogMessage(PK_LOGLEVEL_CRITICAL, "没有在网关配置表 t_forward_list表 中查询到数据：sql：%s", szSql);
		return -1;
	}
	else if (vecRows.size() > 1)
	{
		g_logger.LogMessage(PK_LOGLEVEL_ERROR, "在网关配置表 t_forward_list表 中查询到多条数据：sql：%s,只取第一行", szSql);
	}
...
int CMainTask::PublishRealTagData2Topic(vector<TAGINFO> vecTagsName, vector<string> vecTagsValue, bool bEvent)
{
	...
	root["msg_id"] = GenerateUUID();							 //消息唯一编号，GUID
	...
	root["upload_time"] = getTime();		//添加设备节点,向云端上传数据;
...
int CMainTask::GetEventJsonValue(Json::Value &curDevice, int deviceID, int code, int state)
{
	...
	{
		eventDetail["evt_time"] = getTime();//此处时间为开始组装数据包的时间;
		if (it->action == "记录")			//区分事件的类型，然后组织成完整的数据包;
		{
			if (state == 0)
				eventDetail["evt_name"] = stringToUTF8string(it->name + "恢复");
			else
				eventDetail["evt_name"] = stringToUTF8string(it->name + "启用");
		}
		else
		{
			if (state == 1)
...
int CMainTask::GetCommonJsonValue(vector<string> &vecTagsValue, vector<TAGINFO> &vecTagsName, Json::Value &deviceInfo)
{
	...
	for (int i = 0; i < m_deviceCommon.size(); i++)	//设备数量相对较少，放在外层循环可减少系统消耗;
	{
		//如果是事件设备,则跳过不做处理;
		if (m_deviceCommon[i].driverID == 20000)
			continue;
		//开始组装数据包的时间，也就是从内存数据库获取数据的时间;
		...
		{
			string strDeviceStatus = "device." + m_deviceCommon[i].strName + ".connstatus"; //获取设备状态点位;
			if (m_deviceCommon[i].deviceID == vecTagsName[j].deviceID)						//当前设备的信息;
			{
				...
				if (vecTagsName[j].outputType == "int")//实际上传数据是整型数据;
				{
					long nOut;
					if (vecTagsName[j].calcValue == 0) //数据库中比例系数列未填写或者填写为0;
				...
				else if ((vecTagsName[j].outputType == "float")) // 实际上传数据是浮点型数据;
				{
					float fVal;
					...
					string strPrecision = getRound(fVal, vecTagsName[j].precision);	//获取浮点数保留小数位数;
					if (vecTagsName[j].showType == "electric_meter")
