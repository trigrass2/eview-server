...
int CMqttImpl::mqttClient_Pub(const char* topic, int *mid, int msgLen, const char* szMsg)
{
	if (m_bInit == false)
	{
		g_logger.LogMessage(PK_LOGLEVEL_ERROR, "mqttClient_Pub failed, please init first, topic:%s,len:%d,msg:%s", topic, msgLen, szMsg);
		return MQTT_NO_INITED;
	}

	if (m_bConnected == false)
	{
		g_logger.LogMessage(PK_LOGLEVEL_ERROR, "mqttClient_Pub failed, please connect first, topic:%s,len:%d,msg:%s", topic, msgLen, szMsg);
	...
	if (nRet != 0)
	{
		g_logger.LogMessage(PK_LOGLEVEL_ERROR, "mosquitto_publish failed, return:%d, topic:%s,len:%d,msg:%s", nRet, topic, msgLen, szMsg);
		return MQTT_NO_CONNECTED;
	}
